...
cM <- confusionMatrix(merged_seurat$cfko_slingshot_cluster, merged_seurat$sample)
cM <- cM / rowSums(cM)
pheatmap::pheatmap(cM)


cM <- confusionMatrix(merged_seurat$wt_slingshot_cluster, merged_seurat$sample)
cM <- cM / rowSums(cM)
pheatmap::pheatmap(cM)
...
plotDimRed(merged_seurat, "cfko_slingshot_cluster", plot_type = "rna.umap",
           highlight_group = TRUE, meta_data_col = "cfko_slingshot_cluster",
           group = 0)
